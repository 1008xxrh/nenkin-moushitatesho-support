// 【2026-07-31・2026-07-31決定】genzai側の tests/genzai/test_washing_face_full_flow.js と同じ検証内容を、
// 認定日版(ninteibi)の pages/ninteibi/washing_face.html に対して行う（対象パス・HANDOFF_KEYのみ変更）。
const fs = require('fs');
const { JSDOM } = require('jsdom');
const html = fs.readFileSync(__dirname + '/../../pages/ninteibi/washing_face.html', 'utf-8');
...
(async () => {
  ...
  next();
  doc.querySelectorAll('.option[data-level]')[3].click();
  next();
  assert(doc.querySelector('h1').textContent.includes('「洗面」についてはこれでおわりです'), 'reached complete screen');
  assert(doc.body.textContent.includes('できなかった'), '選んだレベルが完了画面に表示される');
  ...
  assert(window.NEXT_ITEM_URL === 'bathing.html', '次へすすむの行き先が正しい');

  // とじる：handoffが正しいキー(washingFace)・正しい名前空間(adl_handoff_ninteibi_v1)で書かれるか
  ...
  assert(handoffData && handoffData.washingFace, 'ninteibi用handoffにwashingFaceキーで結果が書き込まれている: ' + JSON.stringify(handoffData));
  ...
  console.log(process.exitCode === 1 ? 'NINTEIBI WASHING FACE FULL FLOW: FAILURES ABOVE' : 'NINTEIBI WASHING FACE FULL FLOW: ALL PASSED');
