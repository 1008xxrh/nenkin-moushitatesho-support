// 【2026-07-31・2026-07-31決定】genzai側の tests/genzai/test_eating_full_flow.js と同じ検証内容を、
// 認定日版(ninteibi)の pages/ninteibi/eating.html に対して行う（対象パス・HANDOFF_KEYのみ変更）。
const fs = require('fs');
const { JSDOM } = require('jsdom');
const html = fs.readFileSync(__dirname + '/../../pages/ninteibi/eating.html', 'utf-8');
...
(async () => {
  ...
  assert(doc.querySelectorAll('.found-item').length === 7, 'found items count correct (7グループぶん)');
  next();
  doc.querySelectorAll('.option[data-level]')[2].click();
  next();
  assert(doc.querySelector('h1').textContent.includes('「食事」についてはこれでおわりです'), 'reached complete screen');
  ...
  assert(window.NEXT_ITEM_URL === 'cooking.html', '次へすすむの行き先が正しい');

  // とじる：handoffが正しいキー(eating)・正しい名前空間(adl_handoff_ninteibi_v1)で書かれるか
  ...
  assert(handoffData && handoffData.eating, 'ninteibi用handoffにeatingキーで結果が書き込まれている: ' + JSON.stringify(handoffData));
  ...
  console.log(process.exitCode === 1 ? 'NINTEIBI EATING FULL FLOW: FAILURES ABOVE' : 'NINTEIBI EATING FULL FLOW: ALL PASSED');
