// 【2026-07-31・2026-07-31決定】genzai側の tests/genzai/test_toilet_full_flow.js と同じ検証内容を、
// 認定日版(ninteibi)の pages/ninteibi/toilet.html に対して行う（対象パス・HANDOFF_KEYのみ変更）。
const fs = require('fs');
const { JSDOM } = require('jsdom');
const html = fs.readFileSync(__dirname + '/../../pages/ninteibi/toilet.html', 'utf-8');
...
(async () => {
  ...
  next();
  assert(doc.querySelector('h1').textContent.includes('「トイレ」についてはこれでおわりです'), 'reached complete screen');
  ...
  assert(window.NEXT_ITEM_URL === 'eating.html', '次へすすむの行き先が正しい');

  // とじる：handoffが正しいキー(toilet)・正しい名前空間(adl_handoff_ninteibi_v1)で書かれるか
  ...
  assert(handoffData && handoffData.toilet, 'ninteibi用handoffにtoiletキーで結果が書き込まれている: ' + JSON.stringify(handoffData));
  ...
  console.log(process.exitCode === 1 ? 'NINTEIBI TOILET FULL FLOW: FAILURES ABOVE' : 'NINTEIBI TOILET FULL FLOW: ALL PASSED');
