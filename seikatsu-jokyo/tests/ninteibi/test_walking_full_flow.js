// 【2026-07-31・2026-07-31決定】genzai側の tests/genzai/test_walking_full_flow.js と同じ検証内容を、
// 認定日版(ninteibi)の pages/ninteibi/walking.html に対して行う（対象パス・HANDOFF_KEYのみ変更）。
const fs = require('fs');
const { JSDOM } = require('jsdom');
const html = fs.readFileSync(__dirname + '/../../pages/ninteibi/walking.html', 'utf-8');
...
(async () => {
  ...
  assert(window.NEXT_ITEM_URL === 'laundry.html', '次へすすむの行き先が正しい');

  // とじる：handoffが正しいキー(walking)・正しい名前空間(adl_handoff_ninteibi_v1)で書かれるか
  ...
  assert(handoffData && handoffData.walking, 'ninteibi用handoffにwalkingキーで結果が書き込まれている: ' + JSON.stringify(handoffData));
  ...
  console.log(process.exitCode === 1 ? 'NINTEIBI WALKING FULL FLOW: FAILURES ABOVE' : 'NINTEIBI WALKING FULL FLOW: ALL PASSED');
