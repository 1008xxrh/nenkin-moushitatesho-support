...
const html = fs.readFileSync(__dirname + '/../../pages/genzai/toilet.html', 'utf-8');
...
(async () => {
  ...
  // 次へすすむ：トイレの次は食事（本体ADL_ITEMSの並び順）で、実在するので有効なはず
  const forwardBtn = doc.getElementById('btn-forward');
  assert(forwardBtn !== null && forwardBtn.disabled !== true, '次へすすむが有効になっている（食事の画面が実在するため）');
  assert(window.NEXT_ITEM_URL === 'eating.html', '次へすすむの行き先が食事の画面になっている');

  // とじる：handoffが正しいキー(toilet)で書かれるか
  ...
  assert(handoffData && handoffData.toilet, 'handoffにtoiletキーで結果が書き込まれている: ' + JSON.stringify(handoffData));

  console.log(process.exitCode === 1 ? 'TOILET FULL FLOW: FAILURES ABOVE' : 'TOILET FULL FLOW: ALL PASSED');
