...
const html = fs.readFileSync(__dirname + '/../../pages/genzai/laundry.html', 'utf-8');
...
(async () => {
  ...
  // 次へすすむ：洗濯の次は買物（本体ADL_ITEMSの並び順）で、2026-07-28、買物の画面ができたので有効
  const forwardBtn = doc.getElementById('btn-forward');
  assert(forwardBtn !== null && forwardBtn.disabled !== true, '次へすすむが有効になっている（買物の画面が実在するため）');
  assert(window.NEXT_ITEM_URL === 'shopping.html', '次へすすむの行き先が買物の画面になっている');

  // とじる：handoffが正しいキー(laundry)で書かれるか
  ...
  assert(handoffData && handoffData.laundry === 'まあできる', 'handoffにlaundryキーで結果が書き込まれている: ' + JSON.stringify(handoffData));

  console.log(process.exitCode === 1 ? 'LAUNDRY FULL FLOW: FAILURES ABOVE' : 'LAUNDRY FULL FLOW: ALL PASSED');
